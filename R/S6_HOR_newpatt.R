...
# ---------- helper: safe plot ----------
safe_plot_pdf <- function(path, plot_fun) {
  tryCatch({
    pdf(path, width = 10, height = 10)
    plot_fun()
    dev.off()
  }, error = function(e) {
    message("[skip-plot] ", basename(path), " (", e$message, ")")
    try(dev.off(), silent = TRUE)
  })
}

# ---------- Inputs: BLAT sub summaries ----------
# Allow chr names like chr1, chr10, chrX, chrUn… anything up to the next dot.
data_files <- dir(IN_DIR,
                  pattern = "chr[^.]+\\.cons\\.(9[0-9])\\.blat\\.sub$",
...
for (file in data_files) {
  bx  <- basename(file)
  # chrX.cons.9x.blat.sub  -> threshold 0.9x
  thr <- suppressWarnings(as.numeric(sub(".*\\.cons\\.([0-9]+)\\.blat.*", "\\1", bx)) / 100)
  if (is.na(thr)) {
    message("[skip] cannot parse threshold from: ", bx)
    next
  }

  # ----- robust read -----
  if (file.info(file)$size == 0) { message("[skip] empty: ", bx); next }
  df <- tryCatch(
    read.table(file, sep = "\t", header = FALSE, stringsAsFactors = FALSE,
               quote = "", comment.char = ""),
    error = function(e) { message("[skip] read error ", bx, ": ", e$message); return(NULL) }
  )
  if (is.null(df) || !nrow(df)) { message("[skip] no rows: ", bx); next }
  if (ncol(df) < 5) { message("[skip] bad cols (", ncol(df), "): ", bx); next }

  colnames(df) <- c("match", "nam1", "len1", "nam2", "len2")
  # Basic numeric safety
  suppressWarnings({
    df$match <- as.numeric(df$match)
    df$len1  <- as.numeric(df$len1)
    df$len2  <- as.numeric(df$len2)
  })
  df <- df[complete.cases(df[, c("match","len1","len2")]), ]
  if (!nrow(df)) { message("[skip] no numeric rows: ", bx); next }

  # ----- Jaccard & undirected max -----
  df <- df %>% mutate(jacc = match / (len1 + len2 - match))
  if (!nrow(df)) { message("[skip] no jacc rows: ", bx); next }
  ...
  if (is.null(jac_mat) || !nrow(jac_mat) || !ncol(jac_mat)) {
    message("[skip] cannot build matrix: ", bx); next
  }
  diag(jac_mat) <- 1
  ...
  # count identicals (val==1) in the ORIGINAL matrix
  if (length(keep) > 1) {
  ...
  # Avoid igraph issues with a 0×0 or 1×1 “graph”
  if (!nrow(jac_mat_sub) || !ncol(jac_mat_sub)) { message("[skip] empty thresholded matrix: ", bx); next }

  network <- tryCatch(
    graph_from_adjacency_matrix(jac_mat_sub, mode = "undirected",
                                weighted = TRUE, diag = FALSE),
    error = function(e) NULL
  )
  if (is.null(network) || vcount(network) == 0) {
    message("[skip] empty graph: ", bx)
    next
  }
  ...
  # duplicates inherit best neighbor cluster
  ...
  # Expand seed groups (include identicals at Jaccard==1)
  ...
  if (nrow(nams_group)) {
    ...
    message("[skip-write] no groups for: ", bx)
  }

  # ----- Plot (skip if trivial) -----
  if (vcount(network) >= 1) {
    lay <- layout_with_fr(network)
    lay_rescaled <- tryCatch(norm_coords(lay, ymin = -1, ymax = 1, xmin = -1, xmax = 1),
                             error = function(e) lay)
    ...
    pdf_path <- file.path(plot_dir, paste0(nfile, "_network_plot.pdf"))
    safe_plot_pdf(pdf_path, function() {
      plot(network,
           layout = lay_rescaled,
           rescale = FALSE,
           vertex.size = 8,
...
message("[✓] Done. CSVs in: ", csv_dir, "  | Plots in: ", plot_dir)