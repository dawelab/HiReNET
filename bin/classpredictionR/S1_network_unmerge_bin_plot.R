...
# Always plot in this version
DO_PLOT <- TRUE

if (is.null(INPUT_DIR) || is.null(OUTDIR)) {
  stop("Usage: S1_network_unmerge_bin.R --input DIR --outdir DIR [--lda-model model.rds]\n", call. = FALSE)
}
...
info_summary_all  <- vector("list", length(clust_val))
label_summary_all <- vector("list", length(clust_val))
...
for (file in data_files) {
  ...
  if (length(keep) <= 1) {
    ...
    for (k in seq_along(clust_val)) {
      ...
      label_df <- data.frame(
        bin       = paste0(gsub("\\.$", "", sfile)),
        label     = "A",
        threshold = thr,
        stringsAsFactors = FALSE
      )
      if (is.null(label_summary_all[[k]])) label_summary_all[[k]] <- list()
      label_summary_all[[k]][[length(label_summary_all[[k]]) + 1]] <- label_df
    }
    next
  }
  ...
  for (k in seq_along(clust_val)) {
    ...
    cluster_labels <- c(LETTERS[1:25], 1:9, letters)
    if (kmax <= length(cluster_labels)) cluster_labels <- cluster_labels[1:kmax] else cluster_labels <- paste0("C", seq_len(kmax))
    ...
    # ---- plotting (only in this script; DO_PLOT is TRUE) ----
    if (DO_PLOT) {
      lay <- layout_with_fr(network)
      lay_rescaled <- norm_coords(lay, ymin = -1, ymax = 1, xmin = -1, xmax = 1)

      pdf(file.path(plot_dir, paste0(sfile, "thr", round(thr * 100), "_network_plot.pdf")), width = 10, height = 10)
      plot(network,
           layout = lay_rescaled,
           rescale = FALSE,
           vertex.size = 8,
      ...
      if (kmax > 0) {
        for (cl in seq_len(kmax)) {
          ...
          x <- max(lay_rescaled[vind, 1])
          y <- max(lay_rescaled[vind, 2])
          text(x + 0.05, y + 0.05, labels = cluster_labels[cl], col = "blue", cex = 1.2, font = 2)
        }
      }

      mtext(paste("Monomers", label_string), side = 3, line = 1, adj = 0, col = "black", cex = 1)
...
network_summary_list <- lapply(info_summary_all, dplyr::bind_rows)
label_summary_list   <- lapply(label_summary_all, dplyr::bind_rows)

network_summary_combined <- dplyr::bind_rows(network_summary_list) %>%
  dplyr::arrange(chr, array, start, end, threshold)

label_summary_combined <- dplyr::bind_rows(label_summary_list) %>%
  dplyr::arrange(bin, threshold)

write.table(network_summary_combined,
            file.path(OUTDIR, "network_summary_thresholds.txt"),
            row.names = FALSE, sep = "\t", quote = FALSE)

write.table(label_summary_combined,
            file.path(OUTDIR, "label_summary_thresholds.txt"),
            row.names = FALSE, sep = "\t", quote = FALSE)
...
if (!is.na(LDA_MODEL)) {
  ...
  for (i in seq_along(clust_val)) {
    ...
    if (nrow(network_summary) == 0) next
    ...
    pred <- predict(fit2, newdata = network_summary, type = "class")
    network_summary$lda_pred     <- pred$class
    network_summary$lda_pred_pos <- apply(pred$posterior, 1, max)
  ...
  if (length(network_summary_or_list)) {
    ...
    combined <- data.frame()
    nrows <- nrow(network_summary_or_list[[which.max(sapply(network_summary_or_list, nrow))]])
